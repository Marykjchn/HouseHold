...
type
  ...
  public
    // 양력 -> 음력 변환
    function fnSolar2Lunar( _solDate : TDateTime ) : TDateTime;
    // 음력 -> 양력 변환
    function fnLunar2Solar( _lunDate : TDateTime) : TDateTime;
  published
...
function TSolLun.fnLunar2Solar(_lunDate: TDateTime): TDateTime;
begin
  DecodeDate(_lunDate, FlunYear, FlunMonth, FlunDay);
  Lunar2Solar;
  Result  := EncodeDate( solYear, solMonth, solDay );
end;

function TSolLun.fnSolar2Lunar(_solDate: TDateTime): TDateTime;
begin
  DecodeDate(_solDate, FsolYear, FsolMonth, FsolDay);
  Solar2Lunar;
  Result  := EncodeDate( lunYear, lunMonth, lunDay );
end;
