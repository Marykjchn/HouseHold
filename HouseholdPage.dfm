object HouseHoldForm: THouseHoldForm
  Left = 0
  Top = 0
  HorzScrollBar.ParentColor = False
  BorderStyle = bsDialog
  Caption = '   '#44032' '#44228' '#48512
  ClientHeight = 770
  ClientWidth = 1150
  Color = clWindow
  Font.Charset = HANGEUL_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = #55092#47676#47784#51020'T'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 12
  object StatusBar1: TStatusBar
    Left = 0
    Top = 751
    Width = 1150
    Height = 19
    Panels = <
      item
        Width = 370
      end
      item
        BiDiMode = bdRightToLeft
        ParentBiDiMode = False
        Width = 370
      end
      item
        BiDiMode = bdRightToLeft
        ParentBiDiMode = False
        Width = 371
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1150
    Height = 751
    Align = alClient
    BevelOuter = bvNone
    Color = clWindow
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 4934475
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 2
    object BtnTotalExcel: TImage
      AlignWithMargins = True
      Left = 533
      Top = 438
      Width = 32
      Height = 32
      Cursor = crHandPoint
      Center = True
      Picture.Data = {
        
        00000000}
      OnClick = BtnExcelClick
    end
    object BtnTotalPrint: TImage
      Left = 501
      Top = 438
      Width = 32
      Height = 32
      Cursor = crHandPoint
      Center = True
      Picture.Data = {
        
        00000000}
    end
    object BtnSpendingPrint: TImage
      Left = 876
      Top = 220
      Width = 32
      Height = 32
      Cursor = crHandPoint
      Center = True
      Picture.Data = {
        
        00000000}
    end
    object BtnSpendingExcel: TImage
      Left = 908
      Top = 220
      Width = 32
      Height = 32
      Cursor = crHandPoint
      Center = True
      Picture.Data = {
        
        00000000}
      OnClick = BtnExcelClick
    end
    object BtnIncomeExcel: TImage
      Left = 908
      Top = 4
      Width = 32
      Height = 32
      Cursor = crHandPoint
      Center = True
      Picture.Data = {
        
        00000000}
      OnClick = BtnExcelClick
    end
    object BtnIncomePrint: TImage
      Left = 876
      Top = 4
      Width = 32
      Height = 32
      Cursor = crHandPoint
      Center = True
      Picture.Data = {
        
        00000000}
    end
    object DBPlannerMonthView1: TDBPlannerMonthView
      Left = 5
      Top = 5
      Width = 365
      Height = 425
      Cursor = crDefault
      AttachementGlyph.Data = {
        
        8888888888808880888888888888000888888888888888888888}
      BevelOuter = bvNone
      BorderWidth = 1
      CaptionColor = clWindow
      CaptionColorTo = 16760704
      CaptionHoverColor = 16756834
      CaptionFont.Charset = HANGEUL_CHARSET
      CaptionFont.Color = 3947580
      CaptionFont.Height = -11
      CaptionFont.Name = #47569#51008' '#44256#46357
      CaptionFont.Style = []
      Color = 16776441
      Completion.Font.Charset = DEFAULT_CHARSET
      Completion.Font.Color = clWindowText
      Completion.Font.Height = -11
      Completion.Font.Name = 'Arial'
      Completion.Font.Style = []
      DateFormat = 'dd/mm/yyyy'
      DayFont.Charset = HANGEUL_CHARSET
      DayFont.Color = 4605510
      DayFont.Height = -12
      DayFont.Name = #47569#51008' '#44256#46357
      DayFont.Style = []
      DefaultItem.Alignment = taCenter
      DefaultItem.BorderColor = clGray
      DefaultItem.CaptionAlign = taCenter
      DefaultItem.CaptionFont.Charset = HANGEUL_CHARSET
      DefaultItem.CaptionFont.Color = 3947580
      DefaultItem.CaptionFont.Height = -11
      DefaultItem.CaptionFont.Name = #47569#51008' '#44256#46357
      DefaultItem.CaptionFont.Style = []
      DefaultItem.Color = 16776441
      DefaultItem.ColorTo = 16760704
      DefaultItem.Cursor = -1
      DefaultItem.Font.Charset = DEFAULT_CHARSET
      DefaultItem.Font.Color = 4605510
      DefaultItem.Font.Height = -11
      DefaultItem.Font.Name = #47569#51008#44256#46357
      DefaultItem.Font.Style = []
      DefaultItem.ItemBegin = 16
      DefaultItem.ItemEnd = 17
      DefaultItem.ItemPos = 0
      DefaultItem.Name = 'PlannerItem0'
      DefaultItem.ReadOnly = True
      DefaultItem.SelectColor = 16765348
      DefaultItem.SelectColorTo = 15628032
      DefaultItem.SelectFontColor = clWhite
      DefaultItem.Shadow = True
      DefaultItem.TrackColor = clNone
      DefaultItem.TrackLinkColor = clNone
      DefaultItem.TrackSelectColor = clNone
      DefaultItem.TrackVisible = False
      DeleteGlyph.Data = {
        
        0D0D0D0D0D0D0D0D5EECD9ED070707070707070707070707EDD1}
      FocusColor = 16760704
      Font.Charset = HANGEUL_CHARSET
      Font.Color = 4605510
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      GradientStartColor = 16775150
      GradientEndColor = 16775150
      HintPrevYear = 'Previous Year'
      HintPrevMonth = 'Previous Month'
      HintNextMonth = 'Next Month'
      HintNextYear = 'Next Year'
      InversColor = 16744448
      Items = <>
      LineColor = clWindow
      MonthGradientStartColor = clWindow
      MonthGradientEndColor = 16760181
      NameOfDays.Monday = #50900
      NameOfDays.Tuesday = #54868
      NameOfDays.Wednesday = #49688
      NameOfDays.Thursday = #47785
      NameOfDays.Friday = #44552
      NameOfDays.Saturday = #53664
      NameOfDays.Sunday = #51068
      NameOfDays.UseIntlNames = True
      NameOfMonths.January = '1'
      NameOfMonths.February = '2'
      NameOfMonths.March = '3'
      NameOfMonths.April = '4'
      NameOfMonths.May = '5'
      NameOfMonths.June = '6'
      NameOfMonths.July = '7'
      NameOfMonths.August = '8'
      NameOfMonths.September = '9'
      NameOfMonths.October = '10'
      NameOfMonths.November = '11'
      NameOfMonths.December = '12'
      NameOfMonths.UseIntlNames = True
      PrintOptions.FooterFont.Charset = DEFAULT_CHARSET
      PrintOptions.FooterFont.Color = clWindowText
      PrintOptions.FooterFont.Height = -11
      PrintOptions.FooterFont.Name = 'Tahoma'
      PrintOptions.FooterFont.Style = []
      PrintOptions.HeaderFont.Charset = DEFAULT_CHARSET
      PrintOptions.HeaderFont.Color = clWindowText
      PrintOptions.HeaderFont.Height = -11
      PrintOptions.HeaderFont.Name = 'Tahoma'
      PrintOptions.HeaderFont.Style = []
      SelectColor = 16756834
      ShadowColor = clSilver
      TextColor = 4605510
      TabOrder = 24
      TodayColor = clWindow
      TodayColorTo = 16776441
      TodayStyle = tsRaised
      TrackColor = clYellow
      URLGlyph.Data = {
        
        888887F8F8222208888888776888208888888887777778888888}
      WeekFont.Charset = HANGEUL_CHARSET
      WeekFont.Color = 4605510
      WeekFont.Height = -12
      WeekFont.Name = #47569#51008' '#44256#46357
      WeekFont.Style = []
      WeekName = 'Wk'
      WeekendColor = 16775150
      Day = 22
      Month = 8
      Year = 2018
      Version = '2.5.4.1'
      OnDaySelect = DBPlannerCalendar1DaySelect
      OnMonthChanged = DBPlannerMonthView1MonthChanged
      OnYearChanged = DBPlannerMonthView1YearChanged
    end
    object cxLabel1: TcxLabel
      Left = 15
      Top = 442
      Caption = #47700'  '#47784
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clGray
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.IsFontAssigned = True
    end
    object cxLabel2: TcxLabel
      Left = 381
      Top = 442
      Caption = #44032'  '#44228'  '#48512'   '#45236'  '#50669
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clGray
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.IsFontAssigned = True
    end
    object cxLabel3: TcxLabel
      Left = 799
      Top = 8
      Caption = #49688' '#51077' '#45236' '#50669
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clGray
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.IsFontAssigned = True
    end
    object cxLabel4: TcxLabel
      Left = 799
      Top = 224
      Caption = #51648' '#52636' '#45236' '#50669
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clGray
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.IsFontAssigned = True
    end
    object GRD_Income: TcxGrid
      Left = 789
      Top = 34
      Width = 355
      Height = 180
      Font.Charset = HANGEUL_CHARSET
      Font.Color = 4605510
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      RootLevelOptions.DetailFrameColor = 540818492
      object Simple_Income: TcxGridDBTableView
        NavigatorButtons.ConfirmDelete = False
        OnCellClick = HouseHoldingCellClick
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <
          item
            Format = #52509' ,0'#44148
            Kind = skCount
            Column = Simple_IncomeColumn1
          end
          item
            Format = ',0'#50896
            Kind = skSum
            Column = Simple_IncomeColumn4
          end>
        DataController.Summary.SummaryGroups = <>
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.Footer = True
        OptionsView.GroupByBox = False
        object Simple_IncomeColumn1: TcxGridDBColumn
          Caption = #48264#54840
          DataBinding.FieldName = 'CNUM'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 55
        end
        object Simple_IncomeColumn2: TcxGridDBColumn
          Caption = #54637'  '#47785
          DataBinding.FieldName = 'CITEM'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 68
        end
        object Simple_IncomeColumn3: TcxGridDBColumn
          Caption = #44208#51228' '#49688#45800
          DataBinding.FieldName = 'CMEANS'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 68
        end
        object Simple_IncomeColumn4: TcxGridDBColumn
          Caption = #44552'  '#50529
          DataBinding.FieldName = 'CINCOME'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;-,0.'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 100
        end
      end
      object GRD_IncomeLevel1: TcxGridLevel
        GridView = Simple_Income
      end
    end
    object GRD_Spending: TcxGrid
      Left = 789
      Top = 250
      Width = 355
      Height = 180
      Font.Charset = HANGEUL_CHARSET
      Font.Color = 4605510
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      RootLevelOptions.DetailFrameColor = 540818492
      object Simple_Spending: TcxGridDBTableView
        NavigatorButtons.ConfirmDelete = False
        OnCellClick = HouseHoldingCellClick
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <
          item
            Format = #52509' ,0'#44148
            Kind = skCount
            Column = Simple_SpendingColumn1
          end
          item
            Format = ',0'#50896
            Kind = skSum
            Column = Simple_SpendingColumn4
          end
          item
            Format = ',0'#50896
            Kind = skSum
            Column = Simple_SpendingColumn5
          end>
        DataController.Summary.SummaryGroups = <>
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.Footer = True
        OptionsView.GroupByBox = False
        object Simple_SpendingColumn1: TcxGridDBColumn
          Caption = #48264#54840
          DataBinding.FieldName = 'CNUM'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 45
        end
        object Simple_SpendingColumn2: TcxGridDBColumn
          Caption = #54637'  '#47785
          DataBinding.FieldName = 'CITEM'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 62
        end
        object Simple_SpendingColumn3: TcxGridDBColumn
          Caption = #44208#51228' '#49688#45800
          DataBinding.FieldName = 'CMEANS'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 67
        end
        object Simple_SpendingColumn4: TcxGridDBColumn
          Caption = #51648' '#52636' '#50529
          DataBinding.FieldName = 'CSPENDING'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;-,0.'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 77
        end
        object Simple_SpendingColumn5: TcxGridDBColumn
          Caption = #52852#46300' '#49324#50857#50529
          DataBinding.FieldName = 'CCREDITBAL'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.Alignment.Horz = taRightJustify
          Properties.Alignment.Vert = taVCenter
          Properties.DisplayFormat = ',0.;-,0.'
          FooterAlignmentHorz = taCenter
          Width = 80
        end
      end
      object GRD_SpendingLevel1: TcxGridLevel
        GridView = Simple_Spending
      end
    end
    object GRD_Holding: TcxGrid
      Left = 375
      Top = 467
      Width = 769
      Height = 210
      Font.Charset = HANGEUL_CHARSET
      Font.Color = 4605510
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      RootLevelOptions.DetailFrameColor = 540818492
      object Total_Holding: TcxGridDBTableView
        OnMouseDown = Total_HoldingMouseDown
        NavigatorButtons.ConfirmDelete = False
        NavigatorButtons.Edit.Visible = True
        OnCellClick = HouseHoldingCellClick
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <
          item
            Format = ',0'#50896
            Kind = skSum
            Column = Total_HoldingColumn9
          end
          item
            Format = ',0'#50896
            Kind = skSum
            Column = Total_HoldingColumn8
          end
          item
            Format = ',0'#50896
            Kind = skSum
            Column = Total_HoldingColumn7
          end
          item
            Format = #52509' ,0'#44148
            Kind = skCount
            Column = Total_HoldingColumn1
          end
          item
            FieldName = 'CBALANCE'
            Column = Total_HoldingColumn10
          end>
        DataController.Summary.SummaryGroups = <>
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.Footer = True
        OptionsView.GridLineColor = 541147457
        OptionsView.GroupByBox = False
        OptionsView.NewItemRowSeparatorColor = 541147457
        OptionsView.RowSeparatorColor = 541147457
        object Total_HoldingColumn1: TcxGridDBColumn
          Caption = #48264#54840
          DataBinding.FieldName = 'CNUM'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 59
        end
        object Total_HoldingColumn2: TcxGridDBColumn
          Caption = #45216'  '#51676
          DataBinding.FieldName = 'cdate'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Width = 80
        end
        object Total_HoldingColumn3: TcxGridDBColumn
          Caption = #44396' '#48516
          DataBinding.FieldName = 'CDIV'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 55
        end
        object Total_HoldingColumn4: TcxGridDBColumn
          Caption = #54637'  '#47785
          DataBinding.FieldName = 'CITEM'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          HeaderGlyphAlignmentHorz = taCenter
          Width = 90
        end
        object Total_HoldingColumn5: TcxGridDBColumn
          Caption = #44208#51228' '#49688#45800
          DataBinding.FieldName = 'CMEANS'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 75
        end
        object Total_HoldingColumn6: TcxGridDBColumn
          Caption = #52852#46300#49324
          DataBinding.FieldName = 'CTYPE'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 63
        end
        object Total_HoldingColumn7: TcxGridDBColumn
          Caption = #49688' '#51077' '#50529
          DataBinding.FieldName = 'CINCOME'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;-,0.'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 80
        end
        object Total_HoldingColumn8: TcxGridDBColumn
          Caption = #51648' '#52636' '#50529
          DataBinding.FieldName = 'CSPENDING'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;-,0.'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 80
        end
        object Total_HoldingColumn9: TcxGridDBColumn
          Caption = #52852#46300' '#49324#50857#50529
          DataBinding.FieldName = 'CCREDITBAL'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;-,0.'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 80
        end
        object Total_HoldingColumn10: TcxGridDBColumn
          Caption = #52509'  '#51092' '#50529
          DataBinding.FieldName = 'CBALANCE'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;-,0.'
          FooterAlignmentHorz = taCenter
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Width = 110
        end
      end
      object GRD_HoldingLevel1: TcxGridLevel
        GridView = Total_Holding
        Options.DetailFrameColor = 541147457
      end
    end
    object cxMemo1: TcxMemo
      Left = 5
      Top = 467
      ImeMode = imSHanguel
      ParentFont = False
      Properties.ImeMode = imSHanguel
      Properties.ReadOnly = False
      Properties.ScrollBars = ssVertical
      Properties.OnChange = cxMemo1PropertiesChange
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = 5263440
      Style.Font.Height = -12
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.IsFontAssigned = True
      TabOrder = 5
      Height = 210
      Width = 365
    end
    object BtnAnniversaryEdit: TPanel
      Left = 525
      Top = 690
      Width = 150
      Height = 25
      Cursor = crHandPoint
      Caption = #44592#45392#51068' '#49444#51221
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 8
      OnClick = BtnAnniversaryEditClick
      OnMouseEnter = BtnAnniversaryEditMouseEnter
      OnMouseLeave = BtnAnniversaryEditMouseLeave
    end
    object BtnFixBreakdownEdit: TPanel
      Left = 678
      Top = 690
      Width = 150
      Height = 25
      Cursor = crHandPoint
      Caption = #44256#51221' '#45236#50669' '#49444#51221
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 9
      OnClick = BtnFixBreakdownEditClick
      OnMouseEnter = BtnFixBreakdownEditMouseEnter
      OnMouseLeave = BtnFixBreakdownEditMouseLeave
    end
    object BtnClose: TPanel
      Left = 984
      Top = 690
      Width = 150
      Height = 25
      Cursor = crHandPoint
      Caption = #51333'        '#47308
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 10
      OnClick = BtnCloseClick
      OnMouseEnter = BtnCloseMouseEnter
      OnMouseLeave = BtnCloseMouseLeave
    end
    object BtnReportShow: TPanel
      Left = 831
      Top = 690
      Width = 150
      Height = 25
      Cursor = crHandPoint
      Caption = #48372'  '#44256'  '#49436
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 11
      OnClick = BtnReportShowClick
      OnMouseEnter = BtnReportShowMouseEnter
      OnMouseLeave = BtnReportShowMouseLeave
    end
    object BtnHoldingUpdate: TPanel
      Left = 963
      Top = 445
      Width = 55
      Height = 17
      Cursor = crHandPoint
      Caption = #49688'  '#51221
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 12
      OnClick = BtnHoldingUpdateClick
      OnMouseEnter = BtnHoldingUpdateMouseEnter
      OnMouseLeave = BtnHoldingUpdateMouseLeave
    end
    object BtnHoldingDelete: TPanel
      Left = 1021
      Top = 445
      Width = 55
      Height = 17
      Cursor = crHandPoint
      Caption = #51228'  '#44144
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 13
      OnClick = BtnHoldingDeleteClick
      OnMouseEnter = BtnHoldingDeleteMouseEnter
      OnMouseLeave = BtnHoldingDeleteMouseLeave
    end
    object BtnHoldingAction: TPanel
      Left = 1079
      Top = 445
      Width = 55
      Height = 18
      Cursor = crHandPoint
      Caption = #51312#54924#47784#46300
      Color = clWindow
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = 5263440
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 14
      OnClick = BtnHoldingDeleteClick
    end
    object BtnIncomeDelete: TPanel
      Left = 1021
      Top = 12
      Width = 55
      Height = 17
      Cursor = crHandPoint
      Caption = #51228'  '#44144
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 2
      OnClick = BtnIncomeDeleteClick
      OnMouseEnter = BtnIncomeDeleteMouseEnter
      OnMouseLeave = BtnIncomeDeleteMouseLeave
    end
    object BtnIncomeUpdate: TPanel
      Left = 963
      Top = 12
      Width = 55
      Height = 17
      Cursor = crHandPoint
      Caption = #49688'  '#51221
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
      OnClick = BtnIncomeUpdateClick
      OnMouseEnter = BtnIncomeUpdateMouseEnter
      OnMouseLeave = BtnIncomeUpdateMouseLeave
    end
    object BtnSpendingDelete: TPanel
      Left = 1021
      Top = 228
      Width = 55
      Height = 17
      Cursor = crHandPoint
      Caption = #51228'  '#44144
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 15
      OnClick = BtnSpendingDeleteClick
      OnMouseEnter = BtnSpendingDeleteMouseEnter
      OnMouseLeave = BtnSpendingDeleteMouseLeave
    end
    object BtnSpendingUpdate: TPanel
      Left = 963
      Top = 228
      Width = 55
      Height = 17
      Cursor = crHandPoint
      Caption = #49688'  '#51221
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 16
      OnClick = BtnSpendingUpdateClick
      OnMouseEnter = BtnSpendingUpdateMouseEnter
      OnMouseLeave = BtnSpendingUpdateMouseLeave
    end
    object BtnIncomeAction: TPanel
      Left = 1079
      Top = 12
      Width = 55
      Height = 18
      Cursor = crHandPoint
      Caption = #51312#54924#47784#46300
      Color = clWindow
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = 5263440
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 17
      OnClick = BtnHoldingDeleteClick
    end
    object BtnSpendingAction: TPanel
      Left = 1079
      Top = 228
      Width = 55
      Height = 18
      Cursor = crHandPoint
      Caption = #51312#54924#47784#46300
      Color = clWindow
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = 5263440
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 18
      OnClick = BtnHoldingDeleteClick
    end
    object BtnMemoClear: TPanel
      Left = 300
      Top = 444
      Width = 60
      Height = 20
      Cursor = crHandPoint
      Caption = #52488' '#44592' '#54868
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 19
      OnClick = BtnMemoClearClick
      OnMouseEnter = BtnMemoClearMouseEnter
      OnMouseLeave = BtnMemoClearMouseLeave
    end
    object BtnMemoStore: TPanel
      Left = 174
      Top = 444
      Width = 60
      Height = 20
      Cursor = crHandPoint
      Caption = #51200'   '#51109
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 20
      OnClick = BtnMemoStoreClick
      OnMouseEnter = BtnMemoStoreMouseEnter
      OnMouseLeave = BtnMemoStoreMouseLeave
    end
    object BtnMemoUpdate: TPanel
      Left = 237
      Top = 444
      Width = 60
      Height = 20
      Cursor = crHandPoint
      Caption = #49688'   '#51221
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 21
      OnClick = BtnMemoUpdateClick
      OnMouseEnter = BtnMemoUpdateMouseEnter
      OnMouseLeave = BtnMemoUpdateMouseLeave
    end
    object BtnUpdateMem: TPanel
      Left = 372
      Top = 690
      Width = 150
      Height = 25
      Cursor = crHandPoint
      Caption = #54924' '#50896' '#51221' '#48372
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 25
      OnClick = BtnUpdateMemClick
      OnMouseEnter = BtnUpdateMemMouseEnter
      OnMouseLeave = BtnUpdateMemMouseLeave
    end
  end
  object Panel2: TPanel
    Left = 375
    Top = 5
    Width = 410
    Height = 425
    BevelOuter = bvNone
    Color = 16776441
    ParentBackground = False
    TabOrder = 1
    object cxLabel5: TcxLabel
      Left = 40
      Top = 63
      Caption = #45216'      '#51676
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clBlack
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 4605510
      Style.IsFontAssigned = True
    end
    object cxLabel6: TcxLabel
      Left = 40
      Top = 113
      Caption = #44396'      '#48516
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clBlack
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 4605510
      Style.IsFontAssigned = True
    end
    object cxLabel7: TcxLabel
      Left = 40
      Top = 163
      Caption = #54637'      '#47785
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clBlack
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 4605510
      Style.IsFontAssigned = True
    end
    object cxLabel8: TcxLabel
      Left = 40
      Top = 213
      Caption = #44208#51228' '#49688#45800
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clBlack
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 4605510
      Style.IsFontAssigned = True
    end
    object cxLabel9: TcxLabel
      Left = 40
      Top = 311
      Caption = #44552'      '#50529
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = clBlack
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 4605510
      Style.IsFontAssigned = True
    end
    object cxLabel10: TcxLabel
      Left = 145
      Top = 13
      Caption = #44032' '#44228' '#48512' '#51077#47141
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = 4605510
      Style.Font.Height = -19
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
    end
    object ComboAmount: TcxTextEdit
      Left = 130
      Top = 310
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 5
      Margins.Bottom = 5
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = 4934475
      Style.Font.Height = -12
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 5592405
      Style.IsFontAssigned = True
      TabOrder = 4
      OnKeyPress = ComboAmountKeyPress
      Width = 240
    end
    object ComboType: TcxComboBox
      AlignWithMargins = True
      Left = 130
      Top = 261
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 5
      Margins.Bottom = 5
      ParentFont = False
      Properties.ReadOnly = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = 4934475
      Style.Font.Height = -12
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 5592405
      Style.IsFontAssigned = True
      TabOrder = 12
      Width = 240
    end
    object LabDate: TcxLabel
      Left = 144
      Top = 61
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = 4934475
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.IsFontAssigned = True
    end
    object Panel3: TPanel
      Left = 130
      Top = 98
      Width = 240
      Height = 49
      BevelOuter = bvNone
      TabOrder = 1
      object RadioDiv1: TcxRadioButton
        Left = 20
        Top = 4
        Width = 90
        Height = 40
        Caption = '  '#49688'   '#51077
        Checked = True
        Color = 16776441
        Font.Charset = HANGEUL_CHARSET
        Font.Color = 5592405
        Font.Height = -13
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = []
        ParentColor = False
        ParentFont = False
        TabOrder = 1
        TabStop = True
        OnClick = RadioDivClick
        ParentBackground = False
      end
      object RadioDiv2: TcxRadioButton
        Left = 144
        Top = 4
        Width = 90
        Height = 40
        Caption = '  '#51648'   '#52636
        Color = 16776441
        Font.Charset = HANGEUL_CHARSET
        Font.Color = 5592405
        Font.Height = -13
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = []
        ParentColor = False
        ParentFont = False
        TabOrder = 0
        OnClick = RadioDivClick
        ParentBackground = False
      end
    end
    object Panel4: TPanel
      Left = 130
      Top = 198
      Width = 240
      Height = 51
      BevelOuter = bvNone
      TabOrder = 3
      object RadioMeans1: TcxRadioButton
        Left = 5
        Top = 13
        Width = 60
        Height = 25
        Caption = #54788#44552
        Checked = True
        Color = 16776441
        Font.Charset = HANGEUL_CHARSET
        Font.Color = 5592405
        Font.Height = -12
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = []
        ParentColor = False
        ParentFont = False
        TabOrder = 0
        TabStop = True
        OnClick = RadioMeansClick
        ParentBackground = False
      end
      object RadioMeans2: TcxRadioButton
        Left = 70
        Top = 13
        Width = 75
        Height = 25
        Caption = #52404#53356#52852#46300
        Color = 16776441
        Font.Charset = HANGEUL_CHARSET
        Font.Color = 5592405
        Font.Height = -12
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = []
        ParentColor = False
        ParentFont = False
        TabOrder = 1
        OnClick = RadioMeansClick
        ParentBackground = False
      end
      object RadioMeans3: TcxRadioButton
        Left = 160
        Top = 13
        Width = 75
        Height = 25
        Caption = #49888#50857#52852#46300
        Color = 16776441
        Enabled = False
        Font.Charset = HANGEUL_CHARSET
        Font.Color = 5592405
        Font.Height = -12
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = []
        ParentColor = False
        ParentFont = False
        TabOrder = 2
        OnClick = RadioMeansClick
        ParentBackground = False
      end
    end
    object ComboItem: TcxComboBox
      AlignWithMargins = True
      Left = 130
      Top = 162
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 5
      Margins.Bottom = 5
      ParentFont = False
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = 4934475
      Style.Font.Height = -12
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.IsFontAssigned = True
      TabOrder = 2
      OnExit = ComboItemExit
      Width = 240
    end
    object cxLabel11: TcxLabel
      Left = 40
      Top = 262
      Caption = #52852#46300' '#51333#47448
      ParentColor = False
      ParentFont = False
      Style.Color = 16448250
      Style.Font.Charset = HANGEUL_CHARSET
      Style.Font.Color = 3947580
      Style.Font.Height = -13
      Style.Font.Name = #47569#51008' '#44256#46357
      Style.Font.Style = []
      Style.TextColor = 3947580
      Style.IsFontAssigned = True
    end
    object BtnClear: TPanel
      Left = 210
      Top = 365
      Width = 150
      Height = 30
      Cursor = crHandPoint
      Caption = #52488' '#44592' '#54868
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 6
      OnClick = BtnClearClick
      OnMouseEnter = BtnClearMouseEnter
      OnMouseLeave = BtnClearMouseLeave
    end
    object BtnStore: TPanel
      Left = 50
      Top = 365
      Width = 150
      Height = 30
      Cursor = crHandPoint
      Caption = #51200'     '#51109
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 5
      OnClick = BtnStoreClick
      OnMouseEnter = BtnStoreMouseEnter
      OnMouseLeave = BtnStoreMouseLeave
    end
    object BtnHouseHoldEdit: TPanel
      Left = 310
      Top = 61
      Width = 60
      Height = 21
      Cursor = crHandPoint
      Caption = #49444'  '#51221
      Color = 16752190
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 7
      OnClick = BtnHouseHoldEditClick
      OnMouseEnter = BtnHouseHoldEditMouseEnter
      OnMouseLeave = BtnHouseHoldEditMouseLeave
    end
    object BtnItemDel: TPanel
      Left = 340
      Top = 163
      Width = 30
      Height = 21
      Cursor = crHandPoint
      Caption = #51228#44144
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 9
      OnClick = BtnItemDel2Click
      OnMouseEnter = BtnItemDelMouseEnter
      OnMouseLeave = BtnItemDelMouseLeave
    end
    object BtnTypeDel: TPanel
      Left = 340
      Top = 262
      Width = 30
      Height = 21
      Cursor = crHandPoint
      Caption = #51228#44144
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 11
      OnClick = BtnTypeDelClick
      OnMouseEnter = BtnTypeDelMouseEnter
      OnMouseLeave = BtnTypeDelMouseLeave
    end
    object BtnItemAdd: TPanel
      Left = 307
      Top = 163
      Width = 30
      Height = 21
      Cursor = crHandPoint
      Caption = #52628#44032
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 8
      OnClick = BtnItemAddClick
      OnMouseEnter = BtnItemAddMouseEnter
      OnMouseLeave = BtnItemAddMouseLeave
    end
    object BtnTypeAdd: TPanel
      Left = 307
      Top = 262
      Width = 30
      Height = 21
      Cursor = crHandPoint
      Caption = #52628#44032
      Color = 16760704
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 10
      OnClick = BtnTypeAddClick
      OnMouseEnter = BtnTypeAddMouseEnter
      OnMouseLeave = BtnTypeAddMouseLeave
    end
  end
  object DHContents_SelectAll: TOraDataSource
    Left = 166
    Top = 714
  end
  object QHContents_SelectAll: TOraQuery
    Left = 52
    Top = 714
  end
  object PopupMenu1: TPopupMenu
    Left = 281
    Top = 714
  end
  object Timer1: TTimer
    Left = 16
    Top = 714
  end
  object QHContents_SelectIncome: TOraQuery
    SQLRefresh.Strings = (
      'SELECT cnum, citem, cmeans, cincome '
      'FROM hcontents '
      
        'WHERE cid = :ID AND cdate = :CDATE AND cdiv = :DIV and CNUM = :C' +
        'NUM')
    Left = 91
    Top = 714
  end
  object QHContents_SelectSpending: TOraQuery
    SQLRefresh.Strings = (
      'SELECT cnum, citem, cmeans, cspending, ccreditbal '
      'FROM hcontents '
      
        'WHERE cid = :ID AND cdate = :CDATE AND cdiv = :DIV and CNUM = :C' +
        'NUM')
    Left = 129
    Top = 714
  end
  object DHContents_SelectIncome: TOraDataSource
    Left = 204
    Top = 714
  end
  object DHContents_SelectSpending: TOraDataSource
    Left = 243
    Top = 714
  end
end
