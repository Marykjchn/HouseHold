...
type
  THouseHoldForm = class(TForm)
    ...
    // ----------------- 입력 전처리 -----------------
    // 숫자만 입력가능
    procedure ComboAmountKeyPress(Sender: TObject; var Key: Char);
...
procedure THouseHoldForm.ComboAmountKeyPress(Sender: TObject; var Key: Char);
begin
  ...
  if ComboItem.Text = '' then begin
    ...
    with HoldData do begin
      ...
      if RadioDiv1.Checked then begin
        ...
        cincome   := StrToInt(ComboAmount.Text);
        cspending := 0;
        camount   := cincome;
      // 지출일때 금액 지정
        ...
        cincome   := 0;
        cspending := StrToInt(ComboAmount.Text);
        // 지출이 신용카드가 아닐때 금액 지정
        ...
        // 지출이 신용카드일때 금액 지정
          ...
          cspending        := 0;
          camount          := 0;
          ccreditspending  := StrToInt(ComboAmount.Text);
        end;
